...
task :release_local do
  rake_config = begin
                   YAML.safe_load(File.read("#{ENV['HOME']}/.rake/rake.yml"))
                 rescue
                   {}
                 end
...
task release_all: %i[release_local release_rubygems]

namespace :jenkins do
  task job: %i[build spec_coverage release_all]
end
...
namespace :test do
  ...
  desc 'Build and test manually'
  task :manually do
    Rake::Task['build'].invoke
    cwd = Dir.pwd
    sh 'cd .. && /opt/vagrant/bin/vagrant plugin uninstall vagrant-proxmox || true'
    sh "cd .. && /opt/vagrant/bin/vagrant plugin install #{cwd}/vagrant-proxmox-#{gemspec.version}.gem"
  end
end

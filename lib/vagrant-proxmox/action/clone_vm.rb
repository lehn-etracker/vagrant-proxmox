    ...
    module Action
      ...
      class CloneVm < ProxmoxAction
        ...
        def call(env)
          ...
          env[:ui].warn 'CloneVm called'

          begin
            template_vm_id = connection(env).get_qemu_template_id(config.qemu_template)
            env[:ui].info "found QEMU Template with ID: #{template_vm_id}"
          rescue StandardError => e
            raise VagrantPlugins::Proxmox::Errors::VMCloneError,
                  proxmox_exit_status: "CloneVM: #{e.message} #{e.backtrace.first}"
          end

          begin
            vm_id = connection(env).get_free_vm_id
            env[:ui].info "detected next free vm id: #{vm_id}"
            ...
            @logger.info "CloneVM returned with #{exit_status}"
